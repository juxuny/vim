...
RUN apt-get update && apt-get install -y tzdata git ruby-full gem ruby-dev curl ack-grep build-essential cmake python3-dev openjdk-11-jdk software-properties-common python3-pip python2 vim
...
ADD node-v21.7.0-linux-x64.tar.gz $HOME
ENV PATH=${PATH}:$HOME/node-v21.7.0-linux-x64/bin
